...
Flusso &  	\begin{enumerate}
		...
		\item Il sistema per ogni risorsa nella coda di riproduzione (UC\_43) procede rimuovendola da questa coda (UC\_42);
		\item Il sistema per ogni risorsa associata alla playlist scelta dall'utente il sistema la aggiunge alla coda di riproduzione (UC\_41);
		\end{enumerate}\\\hline
\end{tabular}
...
Priorità & Medium \\\hline
Attori &  UtenteAutenticato \\\hline
Descrizione & Permette di aggiungere alla coda di riproduzione una risorsa\\\hline
Pre-condizioni & L'utente \'e registrato e autenticato correttamente\\\hline
Post-condizioni & La risorsa scelta \'e associata alla coda di riproduzione\\\hline
Flusso &    \vspace{-5mm} \begin{enumerate}
    \item L'utente chiede al sistema l'aggiunta di una risorsa alla coda di riproduzione;
    \item Il sistema chiede all'utente l'informazione riguardo la risorsa da aggiungere;
    \item L'utente fornisce al sistema quale risorsa aggiungere;
    \item Il sistema associa la risorsa alla coda di riproduzione;
    \end{enumerate}\\\hline
\end{tabular}
